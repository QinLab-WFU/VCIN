...
class MultiheadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.0):
        ...
        self.normalize = LayerNorm(embed_dim)
...
class PointwiseFeedForward(nn.Module):
    def __init__(self, emb_dim, fwd_dim, dropout=0.0):
        ...
        self.normalize = LayerNorm(emb_dim)
...
class Classifier(nn.Module):
    ...
    def forward(self, img=None, txt=None, lbl=None, txt_embed=None, pad_mask=None, pad_id=3, threshold=0.5, get_embed=False, get_txt_att=False):
        ...
        if img != None and (txt != None or txt_embed != None):
            ...
            img_features, txt_features = self.mPLUG(img_embedding=img_features, history_embedding=txt_features,
                                                    pad_mask=pad_mask)
...
class Generator(nn.Module):
    def __init__(self, num_tokens, num_posits, embed_dim=128, num_heads=1, fwd_dim=256, dropout=0.1, num_layers=12):
        ...
        self.transform = nn.ModuleList([TransformerLayer(embed_dim, num_heads, fwd_dim, dropout) for _ in range(num_layers)])
        self.attention = MultiheadAttention(embed_dim, 1)
        self.num_tokens = num_tokens
        self.num_posits = num_posits
        bert = AutoModel.from_pretrained("bert")
        bert1_6 = list(bert.children())[1].layer[:6]
        self.BERT=nn.ModuleList(bert1_6)
        self.linear = nn.Linear(embed_dim, 1000)
        
    def forward(self, source_embed, token_index=None, source_pad_mask=None, target_pad_mask=None, max_len=250, top_k=1, bos_id=1, pad_id=3, mode='eye'):
        if token_index != None: # --- Training/Testing Phase ---

            if token_index.shape[1] == 1000:
                token_index=token_index[:,:250]
            if target_pad_mask.shape[1]==1000:
                target_pad_mask=target_pad_mask[:,:250]
            ...
            mask1=pad_mask.unsqueeze(1)
            mask2=att_mask.unsqueeze(0)
            mask=mask2.masked_fill(mask1, float("-inf"))
            mask=mask.unsqueeze(1)

            for layer in self.BERT:
                final_embed = layer(final_embed, mask)
                final_embed=final_embed[0]

            for i in range(len(self.transform)):
                final_embed,_ = self.transform[i](final_embed,pad_mask,att_mask)
            #

            final_embed = final_embed[:, source_embed.shape[1]:, :]

            att = self.linear(final_embed)
            att = nn.Softmax(dim=-1)(att)
            return att,final_embed
...
class ClsGen(nn.Module):
    ...
    def forward(self, image, history=None, caption=None, label=None, threshold=0.15, bos_id=1, eos_id=2, pad_id=3, max_len=250, get_emb=False):
...
class mPLUG(nn.Module):
    def __init__(self):
        ...
        self.fwd_dim=256
        self.num_layers=3
        self.fwd_layers = clones(PointwiseFeedForward(emb_dim=self.embed_dim, fwd_dim=self.fwd_dim, dropout=self.dropout), 4)
        self.attention_layers = clones(MultiheadAttention(embed_dim=self.embed_dim, num_heads=self.num_heads, dropout=self.dropout), 7)
        self.attn=None
    def forward(self,img_embedding,history_embedding,pad_mask):
        self.attn = []
        for i in range(self.num_layers):
            history_embedding,attn = self.attention_layers[i](history_embedding, history_embedding,pad_mask=pad_mask)
            history_embedding,_= self.attention_layers[i + self.num_layers](img_embedding,history_embedding)
            history_embedding = self.fwd_layers[i](history_embedding)
            self.attn.append(_)


        final_embedding = torch.cat([img_embedding, history_embedding], dim=1)
        source_pad_mask = torch.zeros((img_embedding.shape[0], img_embedding.shape[1]),device=img_embedding.device).bool()
        final_pad_mask = torch.cat([source_pad_mask, pad_mask], dim=1)

        final_embedding = self.attention_layers[-1](final_embedding, final_embedding, pad_mask=final_pad_mask)[0]
        final_embedding = self.fwd_layers[-1](final_embedding)

        img_embedding=final_embedding[:,:img_embedding.shape[1],:]
        history_embedding = final_embedding[:, img_embedding.shape[1]:, :]

        return img_embedding ,history_embedding
